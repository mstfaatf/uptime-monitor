...
with no coordination logic at all, since there was only ever one process touching the schedule
and none was needed. Two things changed that:
...
- The lock is held only for the claim stamp (milliseconds), not for the network I/O of the
  actual check, which can take seconds, so a lock is never the bottleneck for check latency.
- `claimed_at` is a plain, visible timestamp column, not an in-memory or session-scoped lock.
  ...
  roughly 2x headroom over the worst realistic single-check duration of up to 5 redirect hops,
...
- The claim query joins `target_region_schedule` to `targets` (to return the target's URL
  ...
  SKIP LOCKED` rather than a bare `FOR UPDATE SKIP LOCKED`. That locks, and skip-contends on,
...
creating a schedule row per known region at target-creation time, isn't available, since the
API has no concept of which regions exist (`REGION` is worker-only configuration, never
written anywhere the API could read it). The worker, the only thing that actually knows its
own region, is the right place to self-register interest in a target it hasn't seen yet.
...
- *No claiming at all.* This is the status quo bug being fixed here. Rejected on its face.
- *Postgres advisory locks* (`pg_advisory_lock`). Considered and rejected: it requires
  inventing and managing a mapping from target IDs to lock key integers, has no built-in
  visibility (an advisory lock doesn't show up in a normal `SELECT` the way `claimed_at` does,
  making it harder to reason about or debug), and still needs a separate mechanism to detect
  and recover an abandoned lock from a crashed session.
- *A dedicated leases/queue table.* Considered and rejected as unnecessary: `SKIP LOCKED` gets
...
- *Keep one shared `next_check_at` / `consecutive_failures` per target.* Considered and
  rejected. This would require inventing a second coordination layer just to fairly rotate
  "whose turn" it is among regions to check a target, and it blends together failure signals
  that are semantically different measurements: a target's reachability from one region and
  from another are two different facts, not two samples of the same fact. Sharing state
  between them defeats the actual purpose of checking from multiple regions in the first
  place.
- *One row per target with a `regions jsonb` column*, instead of a normalized
  `target_region_schedule` table. Considered and rejected. A JSON blob can't be claimed
  per-region with `FOR UPDATE SKIP LOCKED`, since locking the row still locks the whole
  document, so two regions sharing one JSON-column row would contend with each other exactly
  like the single-shared-state option above, just with worse ergonomics for querying "what's
  due."
...
- Two worker instances, whether in the same region or different ones, can run concurrently
  ...
  scheduling tick apart, never the sub-second or near-simultaneous gap an actual double-claim
  would produce.
...
- The dashboard now has to decide how to summarize potentially-divergent per-region data at a
  ...
  solve visually, a badge per region, for instance, rather than solved here by collapsing the
  data before it ever reaches the frontend.
- A target briefly has zero schedule rows for a region it hasn't been seen by yet, until that
  ...
  needs to know what regions exist at all. This is an explicit, accepted tradeoff.
