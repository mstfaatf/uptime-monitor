...
2. **At check time**, independently, the worker re-resolves and re-checks the URL immediately
   before making the request. Since a check can involve following redirects, **every
...
- **Creation-time validation only.** Rejected: this defends nothing against DNS rebinding, where
  a hostname that resolves to a public IP at the moment a user adds it then gets repointed at a
...
- **Following redirects with the HTTP client's own built-in `follow_redirects=True` and
  ...
  each `Location` header before following it. This was caught and fixed directly in this
  codebase, not a hypothetical concern.
...
- **A real credential-leak variant of this same problem was found and fixed after the initial
  ...
  `Authorization` on a cross-origin redirect. A target's basic-auth password or a
...
- The backend's own outbound container needs working DNS resolution for the creation-time check
  to function at all. This is a check that only ever needed to happen on the worker before this
  decision, and it became a new, real requirement on the API service's own network
  configuration once creation-time validation was added.
